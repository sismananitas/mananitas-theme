 	...
 	<div class="gallery-container">
		<div class="sort-section">
			<div class="sort-section-container">
				<div class="sort-handle"><?php esc_html__('Filters', 'pinar')  ?>Selecciona</div>
			...
			<?php 
				foreach ($gallery_items_id as $gallery_item_id) 
				{
					...
					<li class="item col-xs-6 col-md-4 '.(isset($image->post_content) && $image->post_content !=='' ? esc_attr(str_replace(' ', '-', trim(strtolower($image->post_content)))) : '').'">
						<figure>
							<img src="'. str_replace('http://', 'https://', esc_url( $image->guid )).'" alt="11"/>
							<a href="'. str_replace('http://', 'https://', esc_url( $image->guid )).'" class="more-details" data-title="'.esc_attr($image->post_excerpt).'"></a>
							<figcaption>
