...
<section <?= (isset($page_id) && $page_id != '' ? ('id="'.esc_attr( $page_id ) .'"') : '') ?> class="container main-page-container<?= ($page_class !== '' ? esc_attr( $page_class ) : '') ?>">
	<?php if (have_posts()) : ?>
		<?php while (have_posts()) : ?>
			...
			<div class="comments-container">
				<h3>
					<?php comments_number( '', ravis_fn_title_effect(esc_html__( '1 Comment', 'pinar' )), ravis_fn_title_effect(esc_html__( '% Comments', 'pinar' )) ); ?>
				</h3>
				<?php
					comments_template();
